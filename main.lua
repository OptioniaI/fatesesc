...
local setthreadidentity = setthreadidentity or context_set or setthreadcontext or (set_thread_identity)
local getthreadidentity = getthreadidentity or context_get or getthreadcontext or (get_thread_identity)
...
local ExecuteCommand = function(Name, Args, Caller)
    ...
    if (Command) then
        ...
        local Context;
        local sett, gett = context_set or setidentity, context_get or getidentity
...
AddCommand("globalchatlogs", {"globalclogs"}, "enables globalchatlogs", {}, function()
    ...
    if (not Socket) then
        Socket = (websocket or WebSocket).connect("ws://fate0.xyz:8080/scripts/fates-admin/chat?username=" .. LocalPlayer.Name);
...
AddCommand("httplogs", {"httpspy"}, "enables httpspy", {}, function()
    ...
    local Request;
    Request = hookfunction(request or request, newcclosure(function(reqtbl)
        AddLog("request", reqtbl.Url, JSONEncode(Services.HttpService, reqtbl));
...
PluginLibrary = {
    ...
    HasTool = HasTool,
    isSat = isSat,
    Request = request or game.HttpGet,
    CThread = CThread,
